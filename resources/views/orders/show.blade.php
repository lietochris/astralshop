...
@section('title') Detalles de mi compra - Astral Shop @endsection
@section('content')
    <section class="flex-1 flex flex-col bg-gray-300 p-5">
        <div class="flex-1 flex flex-col lg:mx-64">
            <div class="my-2">
                <h2 class="font-semibold text-3xl">Detalles de mi compra</h2>
            </div>

            <div class="bg-white flex flex-col shadow p-5">
                <div>
                    <h2 class="font-semibold text-2xl">Compra #{{$order->id}}
                        - {{ date('d-m-Y', strtotime($order->created_at))}}</h2>
                </div>
                @foreach($order->products as $product)
                    <div class="border-b p-2 mt-5">
                        <div class="my-3">
                ...
                <div class="p-2 mt-5">
                    <div class="my-3">
                        <p class="font-semibold">Dirección de envio</p>
                    </div>
                    <div>
                        <ul>
                            <li class="font-semibold text-gray-800">{{$order->address->street}}</li>
                            <li class="text-gray-700">{{$order->address->zipcode}}</li>
                            <li class="text-gray-700">{{$order->address->suburb}}</li>
                            <li class="text-gray-700">{{$order->address->state}} - {{$order->address->town}}</li>
                            <li class="text-gray-700">Tel: {{$order->address->phone}}</li>
                            <li class="text-gray-700">{{$order->address->additional_directions}}</li>
                        </ul>
                    </div>
                </div>
            </div>
